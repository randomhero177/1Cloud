...
; (function ($, window) {
  ...
  Rate.prototype.build = function () {
     ...
     */
    var base_layer = this.addLayer("base-layer", 100, '\u2606', true);
    var select_layer = this.addLayer("select-layer", selected_width, '\u2605', true);
    var hover_layer = this.addLayer("hover-layer", 0, '\u2605', false);
  ...
  Rate.prototype.addLayer = function (layer_name, visible_width, symbol, visible) {
    ...
    for (var i = 0; i < this.settings.max_value; i++) {
      if (Array.isArray(symbol)) {
        ...
        layer_body += "<span class='fa fa-star'></span>";
      }
      else {
        if (this.settings.convert_to_utf8) {
  ...
  Rate.prototype.updateServer = function () {
    if (this.settings.url != undefined) {
      ...
      var objRating = { HelpTechTitle: this.settings.additional_data, Rate: this.getValue() };

      $.ajax({
        url: this.settings.url,
        type: this.settings.ajax_method,
        data: addRequestVerificationToken('#help-content', objRating),
