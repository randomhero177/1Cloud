...
var Helps = {
    ...
    item: {
        ...
        initPagination: function () {
            for (var i = 0; i < this.config.pagination.length; i++) {
                this.config.pagination[i].addEventListener('click', function (e) {
                    e.preventDefault();
                    Helps.filter.fill(this.dataset.skip, this.dataset.take);
    ...
    modal: {
        config: {
            ...
            previewBtn: '.help-fancy-preview'
        },
        drawSinglePreview: function () {
            $(Helps.modal.config.previewBtn).click(function (e) {
                e.preventDefault();
                var url = $(this).attr('href');
                window.util.fillPreview(Helps.modal.config.previewBlockId, url, Helps.modal.previewCB);
            })
        },
