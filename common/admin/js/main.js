...
var Core = function(options) {
  ...
  var runSideMenu = function(options) {
    ...
    // Count the height of view and fit main content into the space left after filter and pagination
    var fullHeightBlock = function () {
        var mainBlock = $('#main'),
            heightAvailable = mainBlock.height(),
            paginationHeight = (mainBlock.find('.pagination').length == 0) ? 0 : mainBlock.find('.pagination').outerHeight(true),
            filterHeight = (mainBlock.find('#filter-form').length == 0) ? 0 : mainBlock.find('#filter-form').outerHeight(true),
            heightLeft = heightAvailable - filterHeight - paginationHeight - 100;
        $('.main-list__general, .main-list__panel-body').height(heightLeft);
    };
    ...
    $(".nav.sidebar-menu li a").on('click', function (e) {
        if ($('body').hasClass('sb-l-m') && $(this).hasClass('accordion-toggle')) {
            e.preventDefault();
            sidebarLeftToggle();
        }
    });
    ...
    /*
     * set property for localStorage for next page load
    */
    function setMenuStyle() {
      var curMenuStyle = ($('body').hasClass('sb-l-m')) ? 'minified' : 'open';
      localStorage.setItem('curMenuState', curMenuStyle);
    };


    /*
     * check localStorage on page load; set active page and menu style
    */
    function setFromLocalStorage() {
      if (window.location.pathname == '/') {
        if (localStorage['curActivePage']) {
          $.get(localStorage['curActivePage'], function () {
            window.location.href = localStorage['curActivePage'];
          });
        };
      } else {
        localStorage.setItem('curActivePage', window.location.href);
      };
      
      if (localStorage['curMenuState'] == 'open') {
    ...
    $('.sidebar-menu li a.accordion-toggle').on('click', function(e) {
     ...
     // 4.  Check if menu is minified by init and If true prevent it from closing bug
    if ($('body').hasClass('sb-l-m')) {
