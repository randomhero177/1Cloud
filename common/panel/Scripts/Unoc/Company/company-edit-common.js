...
if (typeof getActiveTaskUrl !== 'undefined' && typeof getTaskProgressUrl !== 'undefined') {
    initTaskParallelProgessBar(getActiveTaskUrl, getTaskProgressUrl, showCreateProgressBlock);
};

function showCreateProgressBlock() {
    $.get(getActiveTaskUrl, function (response) {
        if (response.Type == companyCreateTaskId) {
            $('.unoc-steps').removeClass('hidden');
        } else {
            $('.unoc-steps').addClass('hidden');
        }
    });
};