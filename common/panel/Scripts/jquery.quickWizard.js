(function ($) {
    ...
    $.fn.quickWizard = function (options, callback) {
        ...
        var settings = {
            'prevButton': '<button id="form-wizard-prev" class="wizard__btn wizard__prev" type="button">Previous</button>',
            'nextButton': '<button id="form-wizard-next" class="wizard__btn wizard__next" type="button">Next</button>',
            ...
            'containerClass' : 'wizard-container',
            'breadCrumb': true,
            'breadCrumbElement': 'legend',
            'breadCrumbListOpen': '<ol>',
            'breadCrumbListClose': '</ol>',
			'breadCrumbListClass': 'wizard-steps',
            'breadCrumbListElementOpen': '<li>',
            'breadCrumbListElementClose': '</li>',
            'breadCrumbActiveClass': 'current',
            'breadCrumbCompletedClass': 'completed',
        ...
        function disableNext(nextObj){
            if ($(nextObj).is(":button")) {
                $(nextObj).attr('disabled', 'disabled');
            } else {
                $(nextObj).addClass(settings.disabledClass);
            }
        }
        ...
        return this.each(function () {
            ...
            if (settings.breadCrumb) {
                ...
                children.find(settings.breadCrumbElement).each(function (index) {
                    breadCrumbList += settings.breadCrumbListElementOpen + '<span class="number">' + (index + 1) + '. </span>' + $(this).text() + settings.breadCrumbListElementClose;
                });
                ...
                if (settings.stepByStep) {
                    breadCrumbList.children().addClass(settings.disabledClass);
                    breadCrumbList.children().first().removeClass(settings.disabledClass);
                }
